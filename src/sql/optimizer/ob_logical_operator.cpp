...
{
  int ret = OB_SUCCESS;
  if (OB_FAIL(visited_map_.create(10, "HashAllocOp"))) {
...
*/
int AllocOpContext::visit(uint64_t op_id, int64_t op_type)
{
  int ret = OB_SUCCESS;
  ObIArray<uint64_t>* op_ids = visited_map_.get(op_type);
  if (OB_ISNULL(op_ids)) {
    ObSEArray<uint64_t, 8> new_op_ids;
    if (OB_FAIL(visited_map_.set_refactored(op_type, new_op_ids))) {
      LOG_WARN("fail to set_refactored", K(ret));
    } else { 
      op_ids = visited_map_.get(op_type);
    }
  }
  if (OB_SUCC(ret)) {
    if (OB_ISNULL(op_ids)) {
...
*/
bool AllocOpContext::is_visited(uint64_t op_id, int64_t op_type)
{
  bool ret = false;
  ObIArray<uint64_t>* op_ids = visited_map_.get(op_type);
...
int ObLogicalOperator::allocate_material_node_above()
{
  ...
  */
  if (log_op_def::LOG_MATERIAL == type_ ||
    ...
    for (int64_t i = 0; OB_SUCC(ret) && !found_child && i < get_parent()->get_num_of_child(); ++i) {
      if (get_parent()->get_child(i) == this) {
        OZ(get_parent()->allocate_material(i));
        found_child = true;
      }
    }
    ...
    if (OB_FAIL(get_plan()->allocate_material_as_top(top))) {
...
int ObLogicalOperator::find_rownum_expr(bool &found)
{
  ...
  for (auto i = 0; !found && OB_SUCC(ret) && i < get_num_of_child(); i++) {
    OZ(SMART_CALL(get_child(i)->find_rownum_expr(found)));
  }
  return ret;
}
...
int ObLogicalOperator::disable_rownum_expr(hash::ObHashSet<uint64_t> &disabled_op_set, ObIArray<uint64_t> &cur_path)
{
  ...
  if (OB_FAIL(cur_path.push_back(op_id_))) {
    ...
    for (int64_t i = 0; OB_SUCC(ret) && i < get_num_of_child(); i++) {
      OZ(SMART_CALL(get_child(i)->disable_rownum_expr(disabled_op_set, cur_path)));
    }
    cur_path.pop_back();
  }
  return ret;
...
int ObLogicalOperator::recursively_disable_alloc_op_above(AllocOpContext& ctx)
{
  ...
  for (auto i = 0; OB_SUCC(ret) && i < get_num_of_child(); i++) {
    OZ(SMART_CALL(get_child(i)->recursively_disable_alloc_op_above(ctx)));
  }
  return ret;
}
...
int ObLogicalOperator::alloc_op_pre(AllocOpContext& ctx)
{
  ...
  if (OB_ISNULL(my_plan_) || OB_ISNULL(my_plan_->get_stmt()) ||
    ...
    // disable all right child of nlj and spf
    if (OB_SUCC(ret) &&
...
int ObLogicalOperator::alloc_op_at_all_level(AllocOpContext& ctx,
                                             const int64_t &op_type)
{
  int ret = OB_SUCCESS;
  if (ctx.is_visited(op_id_, op_type)) {
    /*do nothing*/
  } else if (op_type == ObAllocOpHint::OB_MATERIAL
             && OB_FAIL(allocate_material_node_above())) {
    LOG_WARN("failed to allocate material above", K(ret));
  } else if (OB_FAIL(ctx.visit(op_id_, op_type))) {
    LOG_WARN("failed to visit alloc op", K(ret));
  } else { /*do nothing*/ }
  return ret;
}

int ObLogicalOperator::alloc_op_at_dfo_level(AllocOpContext& ctx,
                                             const int64_t &op_type) {
  int ret = OB_SUCCESS;
  if (ctx.is_visited(op_id_, op_type)) {
    /*do nothing*/
...
int ObLogicalOperator::alloc_op_at_enumerate_level(AllocOpContext& ctx,
                                                   const ObAllocOpHint& alloc_op_hint) {
  int ret = OB_SUCCESS;
  int64_t op_type = alloc_op_hint.op_type_;
  uint64_t tgt_id = alloc_op_hint.tgt_id_;
  if (op_id_ == tgt_id) {
    if (ctx.is_visited(op_id_, op_type)) {
      /*do nothing*/
    } else if (op_type == ObAllocOpHint::OB_MATERIAL
               && OB_FAIL(allocate_material_node_above())) {
      LOG_WARN("failed to allocate material above", K(ret));
    } else if (alloc_op_hint.is_monitor()
               && OB_FAIL(allocate_monitoring_dump_node_above(op_type, tgt_id))) {
