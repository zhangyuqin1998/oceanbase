                                                  ...
                                                  ObDataType &data_type)
{
  ...
  ObObjType obj_type = static_cast<ObObjType>(parse_tree.int16_values_[0]);
  ...
  for (int64_t i = 0; OB_SUCC(ret) && i < parse_tree.num_child_; ++i) {
    ...
    if (OB_ISNULL(cur_node)) {
      ...
      if (col_type == COL_TYPE_VALUE ||
        ...
        } else if (OB_FAIL(resolve_json_table_check_dup_path(col_def->nested_cols_, cur_col_def->col_base_info_.path_))) {
...
int ObDMLResolver::resolve_alloc_ops(const ParseNode &alloc_op_node, ObIArray<ObAllocOpHint> &alloc_op_hints)
{
  ...
  for (int64_t i = 0; OB_SUCC(ret) && i < alloc_op_node.num_child_; ++i) {
    switch (alloc_op_node.type_) {
      case T_BLOCKING: {
        alloc_op_hint.op_type_ = ObAllocOpHint::OB_MATERIAL;
        break;
      }
      case T_TRACING: {
        alloc_op_hint.op_type_ = ObAllocOpHint::OB_MONITOR_TRACING;
        break;
      }
      case T_STAT: {
        alloc_op_hint.op_type_ = ObAllocOpHint::OB_MONITOR_STAT;
        break;
      }
      default: {
        ret = OB_ERR_UNEXPECTED;
    ...
    if (OB_SUCC(ret)) {
      if (OB_ISNULL(node = alloc_op_node.children_[i])) {
        ...
        // Invalid operator id, do nothing.
          ...
          alloc_op_hint.tgt_id_ = node->value_;
